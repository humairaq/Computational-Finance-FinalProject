...
def download_fundamental_data(input_file_name, output_file_name):
    ...
    for i in symbol:
        ...
        try:
            ...
            totalAssets.append("")
        try:
            totalDebt.append(yfinance.get_long_term_debt() + (yfinance.get_total_current_liabilities() - yfinance.get_account_payable() - yfinance.get_other_current_liabilities()))
        except:
                totalDebt.append("")
        try:
            freeCashFlow.append( yfinance.get_operating_cashflow() + yfinance.get_capital_expenditures())
        except:
            freeCashFlow.append("")
        try:
    ...
    df = pd.DataFrame(list(zip(symbol, sector, epsNext5Y, marketCap, totalAssets, totalDebt, freeCashFlow, beta,peRatio)), 
    columns= ['Symbol', 'Sector','EPS Next 5Y', 'Market Cap', 'Total Assets', 'Total Debt', 'Free Cash Flow', 'Beta', 'P/E Ratio'])
