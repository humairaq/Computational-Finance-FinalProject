...
class DiscountedCashFlowModel(object):
    ...
    def calc_fair_value(self):
        ...
        #TODO
        #1
        yearly_discount_factor = 1/(1+ Stock.lookup_wacc_by_beta(self.stock,Stock.get_beta(self.stock)))
        #2
        FCC = Stock.get_free_cashflow(self.stock)
        #3 
        DCF = 0
        for i in range(1, 6):
         DCF += FCC * math.pow(1+ self.short_term_growth_rate,i)* math.pow(yearly_discount_factor,i)
        #4
        CF5 = FCC * math.pow(1+ self.short_term_growth_rate,5)
        for i in range(1, 6):
            DCF += CF5 * math.pow(1+ self.medium_term_growth_rate,i) * math.pow(yearly_discount_factor,(i+5))
        #5
        CF10 = CF5 * math.pow(1+ self.medium_term_growth_rate,5)
        for i in range(1, 11):
            DCF += CF10 * math.pow(1+ self.long_term_growth_rate,i) * math.pow(yearly_discount_factor,(i + 10))
        #6
        PV = Stock.get_cash_and_cash_equivalent(self.stock) - Stock.get_total_debt(self.stock) + DCF
        #7
        result = PV/Stock.get_num_shares_outstanding(self.stock)
        #end TODO
        return(result)
